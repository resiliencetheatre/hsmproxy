...
DAEMON = src/main.c src/config.c src/hsm.c src/channel_test.c
PREFIX ?= /usr/local
.PHONY: all check sanitize clean install
all: hsmproxy
hsmproxy: $(CORE) $(DAEMON) src/core.h src/config.h src/hsm.h src/channel_test.h
...
check: hsmproxy tests/test_core tests/test_hsm
	./tests/test_core
	./tests/test_hsm
...
clean:
	rm -f hsmproxy tests/test_core tests/test_hsm tests/mock_pkcs11.so tests/test_sanitize tests/test_hsm_sanitize
